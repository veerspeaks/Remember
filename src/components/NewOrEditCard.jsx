...
const NewOrEditCard = ({ params }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(e.response?.data?.message || 'An error occurred while creating the card');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center p-3 sm:p-4 w-full max-w-3xl mx-auto mt-4 sm:mt-8 gap-6 sm:gap-8">
      <div className="w-full flex justify-between items-center px-2">
        ...
        <h1 className={`${geistMono.variable} font-bangers text-2xl sm:text-3xl text-yellow-500`}>New Card</h1>
